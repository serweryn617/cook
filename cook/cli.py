...
class Cli:
    ...
    def generate_template(self) -> StepResult:
        ...
        if recipe_path.is_file():
            log(
                f"recipe_template{self.template}.py already present in {self.recipe_base_path}",
                "info",
            )
            return 1
        ...
        log(
            f"recipe_template{self.template}.py generated in {self.recipe_base_path}",
            "log",
        )
        return 0
    ...
    def search_for_recipe(self) -> StepResult:
        ...
        for directory in dirs_to_check:
            ...
            if recipe_path.is_file():
                ...
                return
        log(
            f"recipe file not found in {self.recipe_base_path} or parent directories!",
            "info",
        )
        return 1
...
def cli() -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="?",
        help="Project to build. Uses value of `default_project` if left unspecified.",
    )
    parser.add_argument(
        "-b",
        "--build_server",
        help="Build server to use. Uses value of `default_build_server` if left unspecified.",
    )
    parser.add_argument(
        "-i",
        "--interactive",
        action="store_true",
        help="Use interactive project and build server selection.",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="User arguments. Can be used in recipe file. Format either `key=value` or `flag`.",
    )
    parser.add_argument(
        "-l",
        "--list",
        action="store_true",
        help="List available projects and build servers.",
    )
    parser.add_argument(
        "-t",
        "--template",
